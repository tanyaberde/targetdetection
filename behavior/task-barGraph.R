...
table(Subject, TaskType) # Check number of subjs/cells per condition
...
# 
# meanSubxTasktype=ddply(data,.(Subject,TaskType),summarize,
#                   value=mean(Stim.RT))

mdSubxTasktype=ddply(data,.(Subject,TaskType),summarize,
                       value=median(Stim.RT))

### Change handle
# statFocus <- meanSubxTasktype
statFocus <- mdSubxTasktype

# Load Rmisc package to run summarySE function. Used to be # source("summarySE.R")
require(Rmisc)

TaskTypebar <- summarySEwithin(statFocus,
                             measurevar="value",
                             withinvars=c("TaskType"),
                             idvar = "Subject")
...
a.raw <- ggplot(TaskTypebar, aes(x=TaskType, y=value, fill=TaskType
                               )) + 
...
ggsave("pictures/Task md.png", ### Image file title
       a.official, width=4, height=8)
